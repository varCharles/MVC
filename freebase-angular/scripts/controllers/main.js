...
angular.module('googleApp')
  	.controller('MoviesCtrl', function ($scope, $http) {
   		$scope.getQuery = function(){
	    	$http.jsonp('https://www.googleapis.com/freebase/v1/search?query=' + $scope.freebaseQuery +'&filter=(all+type%3A%2Ffilm%2Ffilm)&callback=JSON_CALLBACK').success(
			function( payload ){
  ...
  .controller('MovieCtrl', function ($scope, $http, $routeParams) {
    	...
    	$http.jsonp('https://www.googleapis.com/freebase/v1/topic/m/'+ $scope.movieid +'?callback=JSON_CALLBACK').success(
			function( payload ){
	...
	.controller('ActorCtrl', function ($scope, $http, $routeParams) {
    	...
    	$http.jsonp('https://www.googleapis.com/freebase/v1/topic/m/'+ $scope.actorid +'?callback=JSON_CALLBACK').success(
			function( payload ){
